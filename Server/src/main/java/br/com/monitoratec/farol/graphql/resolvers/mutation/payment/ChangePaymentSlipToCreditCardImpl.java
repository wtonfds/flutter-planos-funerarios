...
@Component
public class ChangePaymentSlipToCreditCardImpl extends BaseResolver implements GraphQLMutationResolver {
    private static final Logger LOGGER = LoggerFactory.getLogger(ChangePaymentSlipToCreditCardImpl.class);

    private final ClientRepository clientRepository;
    private final PaymentService paymentService;
    private final SubscribedPlanRepository subscribedPlanRepository;

    public ChangePaymentSlipToCreditCardImpl(ClientRepository clientRepository, PaymentService paymentService, SubscribedPlanRepository subscribedPlanRepository) {
        this.clientRepository = clientRepository;
        this.paymentService = paymentService;
        this.subscribedPlanRepository = subscribedPlanRepository;
    }

    public CompletableFuture<CommonResponse> changePaymentSlipToCreditCard(CardInput cardInput, DataFetchingEnvironment dataFetchingEnvironment) {
        ...
        super.validateUserPromise(dataFetchingEnvironment).bindToResponsePromise(responsePromise).anyOf(Arrays.asList(AccessingEntity.values()))
                .thenAccept(cachedTrustedToken -> {
                    ...
                    if (!optionalSubscribedPlan.get().getPaymentType().equals(PaymentTypeDTO.PAYMENT_SLIP.name())) {
                        throw new ErrorOnProcessingRequestException(StatusCodes.Error.Payment.PAYMENT_METHOD_IS_NOT_PAYMENT_SLIP);
                    }

                    paymentService.changePaymentSlipToCreditCard(cardInput, optionalClient.get(), optionalSubscribedPlan.get());
